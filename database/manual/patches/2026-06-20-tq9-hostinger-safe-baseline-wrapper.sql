...
--
-- Production safety:
-- - removes CREATE DATABASE and USE because Hostinger creates DBs in panel;
-- - must be run only after read-only identity evidence shows selected DB is u504065335_to_quran on Hostinger;
-- - keeps the baseline empty-table preflight guard;
-- - structure-only: no Week14 rows/users are imported.

-- To Quran real app DB schema transition baseline
-- Date: 2026-05-28
...
-- Dry-run proof: database/manual/patches/2026-05-28-create-toquranapp-local-baseline.sql created `toquranapp_local` with 352 tables and no imported rows.
...
-- Safety:
--   - This patch intentionally targets `u504065335_to_quran` for accelerated To Quran deployment.
--   - This baseline aborts if `u504065335_to_quran` already contains tables.
