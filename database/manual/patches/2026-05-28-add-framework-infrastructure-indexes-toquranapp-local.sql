...
DROP PROCEDURE IF EXISTS `_toquran_assert_no_orphans`$$
CREATE PROCEDURE `_toquran_assert_no_orphans`(
    IN p_label VARCHAR(128),
    IN p_orphan_query TEXT
)
BEGIN
    SET @toquran_orphan_count = 0;
    SET @toquran_orphan_sql = CONCAT(
        'SELECT COUNT(*) INTO @toquran_orphan_count FROM (',
        p_orphan_query,
        ') AS orphan_rows'
    );

    PREPARE toquran_orphan_stmt FROM @toquran_orphan_sql;
    EXECUTE toquran_orphan_stmt;
    DEALLOCATE PREPARE toquran_orphan_stmt;

    IF @toquran_orphan_count > 0 THEN
        SET @toquran_orphan_message = CONCAT(
            'ABORTED: orphaned rows found for ',
            p_label,
            '; clean data before adding the framework foreign key.'
        );
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = @toquran_orphan_message;
    END IF;
END$$
...
CALL `_toquran_assert_no_orphans`('model_has_permissions.permission_id', 'SELECT `model_has_permissions`.`permission_id` FROM `model_has_permissions` LEFT JOIN `permissions` ON `permissions`.`id` = `model_has_permissions`.`permission_id` WHERE `permissions`.`id` IS NULL LIMIT 1')$$
CALL `_toquran_assert_no_orphans`('model_has_roles.role_id', 'SELECT `model_has_roles`.`role_id` FROM `model_has_roles` LEFT JOIN `roles` ON `roles`.`id` = `model_has_roles`.`role_id` WHERE `roles`.`id` IS NULL LIMIT 1')$$
CALL `_toquran_assert_no_orphans`('role_has_permissions.permission_id', 'SELECT `role_has_permissions`.`permission_id` FROM `role_has_permissions` LEFT JOIN `permissions` ON `permissions`.`id` = `role_has_permissions`.`permission_id` WHERE `permissions`.`id` IS NULL LIMIT 1')$$
CALL `_toquran_assert_no_orphans`('role_has_permissions.role_id', 'SELECT `role_has_permissions`.`role_id` FROM `role_has_permissions` LEFT JOIN `roles` ON `roles`.`id` = `role_has_permissions`.`role_id` WHERE `roles`.`id` IS NULL LIMIT 1')$$
