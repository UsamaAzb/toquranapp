...
<head>
  ...
  <meta name="description"
    content="@yield('meta_description', config('variables.templateDescription') ? config('variables.templateDescription') : '')" />
  <meta name="keywords"
    content="@yield('meta_keywords', config('variables.templateKeyword') ? config('variables.templateKeyword') : '')" />
  <meta property="og:title" content="@yield('title', config('variables.ogTitle') ? config('variables.ogTitle') : '')" />
    ...
    Livewire.hook('request', ({ fail }) => {
        fail(({ status, preventDefault }) => {
            if (status === 401 || status === 419) {
                preventDefault(); // يمنع popup "page expired"
