...
@section('title', 'Series Tasks')
@section('meta_description', 'Choose a subject to manage scheduled Series Tasks and ordered Library delivery.')
...
<style>
  ...
  .series-subject-card:hover,
  .series-subject-card:focus-within {
...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-6">
  <div>
    <h4 class="mb-1">Series Tasks</h4>
    <p class="mb-0 text-body-secondary">Choose a subject to sequence Library content for scheduled delivery.</p>
  </div>
...
<div class="row g-4">
  @forelse($subjects_list as $i => $item)
    ...
    <div class="col-12 col-sm-6 col-xl-4">
      <a href="{{ route('series-tasks.board', ['auth_role' => $role, 'subject' => $item->id]) }}"
         class="text-decoration-none text-reset"
         aria-label="Manage Series Tasks for {{ $item->title }}">
        <div class="card series-subject-card h-100">
          <div class="card-body d-flex flex-column gap-4">
            <div class="d-flex align-items-start justify-content-between gap-3">
              <span class="series-subject-icon avatar-initial rounded bg-label-{{ $colorClass }} text-{{ $colorClass }} d-inline-flex align-items-center justify-content-center">
                {{ $initial }}
              </span>
              <span class="badge bg-label-secondary">Series Tasks</span>
            </div>
            <div>
              <h5 class="mb-1 text-truncate" title="{{ $item->title }}">{{ $item->title }}</h5>
              <p class="mb-0 text-body-secondary small">Ordered Library items by student pathway</p>
            </div>
