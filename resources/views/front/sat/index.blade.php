...
@section('title', 'SAT Practice')
@section('meta_description', 'Browse SAT reading and writing practice sets.')
@section('content')
...
<div class=" row  mt-2 justify-content-center">
  ...
  <div class="accordion unit_group" id="accordionExample">
    ...
    @if($sats)
    @foreach($sats as $k=>$sat)
    @if($sat->parent_id==0)
    
    <?php $sat_id=$sat->id;  ?>
    <div class="accordion-item panel panel-default">
      <h2 class="accordion-header panel-heading " id="heading_{{$sat_id}}" >

      
        
        
         <button class="accordion-button unit-button @if($k!=0) collapsed @endif col-md-9" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_{{$sat_id}}" aria-expanded="@if($k!=0) true @else false @endif" aria-controls="collapse_{{$sat_id}}">
         <p>{{$sat->title}} </p> 
      ...
      <div id="collapse_{{$sat_id}}" class="accordion-collapse collapse  @if($k==0) show @endif " aria-labelledby="heading_{{$sat_id}}" data-bs-parent="#accordionExample">
        <div class="accordion-body panel-body lessons_body">
         

          @if($sat->sat_desc)
          @foreach($sat->sat_desc as $k=>$sat_desc)
<a  @if($sat_desc->slug!= Null) href="{{url('course/sat/'.$sat->slug.'/'.$sat_desc->slug)}}" @else href="#"  @endif>
