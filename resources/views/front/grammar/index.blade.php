...
@section('title', 'Grammar Practice')
@section('meta_description', 'Browse grammar references and practice pathways.')
@section('content')
...
<div class=" row  mt-2 justify-content-center">
  ...
  <div class="accordion unit_group" id="accordionExample">
    ...
    @if($grammars)
    @foreach($grammars as $k=>$grammar)
    @if($grammar->parent_id==0)
    
    <?php $grammar_id=$grammar->id;  ?>
    <div class="accordion-item panel panel-default">
      <h2 class="accordion-header panel-heading " id="heading_{{$grammar_id}}" >

      
        
        
         <button class="accordion-button unit-button  collapsed  col-md-9" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_{{$grammar_id}}" aria-expanded=" false " aria-controls="collapse_{{$grammar_id}}">
         <p>{{$grammar->title}} </p> 
      ...
      <div id="collapse_{{$grammar_id}}" class="accordion-collapse collapse   " aria-labelledby="heading_{{$grammar_id}}" data-bs-parent="#accordionExample">
        <div class="accordion-body panel-body lessons_body">
         

          @if($grammar->grammar_desc)
          @foreach($grammar->grammar_desc as $k=>$grammar_desc)
<a  @if($grammar_desc->slug!= Null) href="{{url('course/grammar/'.$grammar_desc->slug)}}" @else href="#"  @endif>
