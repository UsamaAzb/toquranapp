...
<body>
    ...
    @endphp
    @php
        $audioPath = (string) ($chapter->audio ?? '');
    ...
    @if(!empty($chapter->audio) && !empty($chapter->iframe_link))
   ...
   <!--<a class="back_but" href="{{url('reading/listen-read?d='.$story_id)}}">Go Back</a>-->
...
<div class="audio-div">
  ...
  <source src="{{asset($chapter->audio)}}" type="{{ $audioMimeType }}">
Your browser does not support the audio element.
    ...
    @elseif(!empty($chapter->iframe_link) && empty($chapter->audio))
        <!--<a class="back_but" href="{{url('reading/listen-read?d='.$story_id)}}">Go Back</a>-->
             <a class="back_but" href="{{url('reading/listen-read?d='.$story_id)}}" data-return-url="{{ $resourceReturnTarget }}" onclick="return window.w14ResourceBack(event, this.href);">Go Back</a>
