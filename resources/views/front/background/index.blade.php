...
@section('meta_description', 'Browse background readings and context-building materials.')
@section('content')
...
<div class=" row  mt-2 justify-content-center">
  ...
  <div class="accordion unit_group" id="accordionExample">
    ...
    @if($background->parent_id==0)
    
    <?php $background_id=$background->id;  ?>
    <div class="accordion-item panel panel-default">
      <h2 class="accordion-header panel-heading " id="heading_{{$background_id}}" >

      
        
        
         <button class="accordion-button unit-button  collapsed  col-md-9" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_{{$background_id}}" aria-expanded=" false " aria-controls="collapse_{{$background_id}}">
      ...
      <div id="collapse_{{$background_id}}" class="accordion-collapse collapse   " aria-labelledby="heading_{{$background_id}}" data-bs-parent="#accordionExample">
        <div class="accordion-body panel-body lessons_body">
         

          @if($background->background_desc)
          @foreach($background->background_desc as $k=>$background_desc)
<a  @if($background_desc->slug!= Null) href="{{url($background_desc->slug)}}" @else href="#"  @endif>
          <div class="lesson_div ">

{{$background_desc->title}}
          </div>
          </a>
          @endforeach
          @endif
