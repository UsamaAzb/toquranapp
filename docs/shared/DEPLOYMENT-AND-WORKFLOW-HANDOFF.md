...
- App transfer provisioning now activates class-subject rows based on each child's selected public `service_interests`, in addition to the fixed active-by-default Quranic Arabic, My Deen Journey, and Well Being subjects. For example, a child selecting Arabic Language should receive an active Arabic Language subject/class row after transfer.
