...
5. Public `toquran` repo handoff under TQ9 is implemented in website commit `6dfb71f`: booking form values, multi-child/per-child multi-service intake payload, reference prefix, Contact Us behavior, contact phone `+201091051913`, sign-in link, and the Week14-style shared app DB write path.
6. Local production-equivalent end-to-end smoke passed on 2026-06-02 using the real app DB name `u504065335_to_quran`: public website clean booking, duplicate/review booking, generic Contact Us, app transfer, parent/student/teacher login, and teacher class visibility.
7. Complete the remaining deployment hardening: production backup/export, queue/mail settings, storage/build asset verification, JavaScript dependency audit strategy, smoke-data cleanup, and temporary credential rotation. Composer security advisories are clear as of 2026-06-02.
...
- The reviewed app-owned `contacts.child_age` nullable patch has been executed locally against `u504065335_to_quran` with target verification and structure-backup evidence. Production deploy/import must preserve `contacts.child_age NULL DEFAULT NULL`; generic Contact Us rows should not need or fake child age.
...
- App-side class-subject catalog: Quran Memorization, Arabic Language, Quranic Arabic, Sanad Program, Islamic Studies, Quran Literature, My Deen Journey / `MDJ`, and Well Being.
- Public child-facing intake service selector after the 2026-06-21 app-first expansion: Quran Memorization, Quranic Arabic, Arabic Language, Islamic Studies, Quran Literature, Sanad Ijazah Program, and My Deen Journey. Each child may choose one or more services. The public website must still apply its matching form update before exposing the two new options.
- App transfer provisioning now activates selected-only class-subject rows based on each child's selected public `service_interests`; Quran Memorization, Quranic Arabic, Arabic Language, Sanad Program, Islamic Studies, and Quran Literature are not active unless selected or manually activated. My Deen Journey and Well Being are the fixed active-by-default student surfaces. For example, a child selecting Arabic Language should receive an active Arabic Language subject/class row after transfer.
