...
Use this local app DB target for Phase 2:
...
Forbidden target for app baseline work:

```text
u504065335_to_quran
```

That name belongs to the public/live website export evidence and must not receive app baseline imports by accident.
...
Before executing DB setup, create durable artifacts under `database/manual/`:
...
2. `database/manual/patches/YYYY-MM-DD-toquran-starter-reference-data.sql`
   - roles/permissions;
   - service catalog/reference rows;
   - minimal app defaults.
3. `database/manual/patches/YYYY-MM-DD-toquran-legacy-video-library-mapping-notes.sql`
   - notes only at first, unless TQ6 Library migration is explicitly started.
4. Optional cleanup plan doc before any destructive cleanup of old/export-only data.
...
- target DB is verified as `toquranapp_local`;
- connection is local XAMPP/app DB, not public/live website;
- manual SQL or execution notes exist under `database/manual/`;
- command does not drop/truncate/overwrite the public website DB;
...
- Production DB naming/deployment is not decided by this local target name.
