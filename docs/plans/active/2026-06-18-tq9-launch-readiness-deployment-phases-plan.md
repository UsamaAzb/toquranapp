...
- public website shared-DB handoff is implemented in `toquran` commit `6dfb71f`;
...
- current PHP/Composer local tooling is available, but JavaScript tooling is incomplete because the repo has `yarn.lock` and no local `yarn` command;
- `composer audit` and `composer audit --no-dev` now report one medium Laravel advisory against locked `laravel/framework` `v12.61.0`; launch should update to a fixed version such as `>=12.61.1` and re-test;
...
- local public website repo is `https://github.com/UsamaAzb/toquran.git` on `main`, but the local worktree currently has uncommitted changes. Website deployment must not assume GitHub is current until those changes are reviewed, committed, and pushed or explicitly discarded by owner decision;
...
4. Resolve JavaScript audit strategy:
   - preferred: install/use Yarn compatible with existing `yarn.lock`;
   - alternative: owner-approved migration to a different lockfile tool;
   - do not create `package-lock.json` casually during launch.
5. Run the intended JS audit command after the lockfile/tool decision.
6. Rebuild assets with the intended package manager.
...
8. Prepare the public website repo for deployment before Hostinger work:
   - review the local `D:\xampp\htdocs\toquran` diff;
   - decide whether to keep, amend, or discard the current uncommitted website changes;
...
- JS audit decision documented and audit result recorded;
...
- website repo dirty/clean state and pushed commit are recorded before deployment;
...
7. Produce a concrete production DB manifest before any DB action. The manifest must list exact filenames/commands in execution order and classify each step as:
   - schema baseline;
   - schema correction;
   ...
   - cleanup;
   - verification only.
8. Update `database/manual/README.md` replay/order notes before production replay if the current order is stale.
9. Confirm which manual artifacts must be present on production, including:
...
10. For each DB-changing action, require:
...
11. Do not run Laravel migrations, seeders, `migrate:fresh`, `db:wipe`, restore/import, or destructive SQL unless separately approved with the required DB safety evidence.
12. Validate backup/restore confidence before production launch:
...
6. Public website booking creates app-owned booking rows with `TQ-` references.
7. Duplicate/review booking routes to intake review, not normal booking.
8. Contact Us creates `CNT-` row in `contacts` with nullable child age.
9. App review/transfer creates parent/student users and assigns the configured default teacher.
10. Teacher class/session/task creation works.
11. Library attachment snapshots render for teacher/student/parent.
12. My Deen Journey, rewards/points, behavior, and consequence agreement surfaces render and save.
13. TQ7/TQ7.5 automation assignment generates visible student/parent work.
14. Old Week14 Library/Vocabulary surfaces remain hidden or denied.
15. Scheduler and queue/mail behavior are verified with safe launch checks.
...
5. Rotate any real staff/admin/teacher credentials that were set to shared local test passwords.
...
The public website handoff is already implemented in `toquran` commit `6dfb71f`, but deployment target verification remains required:
...
- Composer audit currently fails because `laravel/framework` `v12.61.0` is affected by a medium advisory reported on 2026-06-17.
- JS audit is unresolved because the repo has `yarn.lock` and no local Yarn command.
...
- Public website deployment uses `UsamaAzb/toquran`, but local website changes must be reviewed/committed/pushed before relying on Hostinger GitHub pull.
