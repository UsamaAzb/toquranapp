...
- Status: `done`
- Depends on: TQ1.5
- Goal: Adapt Week14 family/intake model to To Quran service interests. App launch supports Quran Memorization, Quranic Arabic, Arabic Language, Islamic Studies, Quran Literature, My Deen Journey, Paid Parental Consultation, and Sanad Ijazah; public child-facing intake should expose the owner-approved child services listed under TQ9.
...
- App action: use the admin/superadmin Starter Catalog UI to preview and install the catalog for selected active teachers, then verify teachers can assign the copied starter routines/series.
...
- App action: keep Composer audit clean, keep app production JavaScript audit clean using the existing `yarn.lock` via `corepack yarn 1.22.22`, confirm production `.env`, build assets, storage link, queue/mail behavior, first superadmin/admin/teacher access, and final app smoke.
- Website action: public website shared-DB handoff was introduced in `toquran` commit `6dfb71f`: booking writes use the Week14-style app-owned tables (`bookings`, `booking_children`, `booking_intake_review`, `booking_intake_review_children`, and `booking_intake_submission_locks`), Contact Us targets app-compatible `contacts`, booking references use `TQ-`, contact references use `CNT-`, and the launch public phone is `+201091051913`. Current website deployment candidate is `24d76d1` pending Booking/Contact controller review against the app-owned intake contract. Remaining website work is dependency-audit cleanup or explicit risk acceptance, deployment smoke/verification against the intended shared app DB target, and DB credential-scope decision.
- Current artifacts: local production-equivalent smoke and hardening evidence is in `docs/audits/2026-06-02-tq9-shared-db-smoke-and-hardening.md`; deployment phase plan is in `docs/plans/active/2026-06-18-tq9-launch-readiness-deployment-phases-plan.md`. HTTP parent/student/teacher login smoke passed after the shared website booking/contact flow. App Composer advisory from 2026-06-17 was patched on 2026-06-19 by updating `laravel/framework` to `v12.62.0`; app Composer audits and Unit/Feature tests passed. App JavaScript tooling decision is `corepack yarn 1.22.22` with the existing `yarn.lock`; app production dependency audit is clean after the 2026-06-19 hardening pass, `corepack yarn build` passed, and `docs/BUILD-DEPLOY-MARKER.md` was updated. Full JS audit still has dev/tooling-only advisories, so production deployment must avoid installing dev dependencies or running server-side builds unless separately fixed or risk-accepted. Public website Composer/npm audits also block production-changing deployment until fixed or explicitly risk-accepted.
