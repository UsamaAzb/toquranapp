...
- Website action: adapt the public website to the Week14 shared-table handoff pattern. Public booking must write directly to app-owned intake tables in the shared app DB, not to a delayed JSON import queue: `bookings`, `booking_children`, `booking_intake_review`, `booking_intake_review_children`, and `booking_intake_submission_locks`. Contact Us must write to app-compatible `contacts`, not legacy `contact_us`. Keep booking values, reference prefix, sign-in link, and public contact phone number `+201091051913` aligned; do not promise automated scheduling, finance, or full class management. Public booking must stay multi-child and each child must be able to select one or more child-facing services: Quran Memorization, Quranic Arabic, Arabic Language, Sanad Ijazah Program, and My Deen Journey.
- DB action: confirm real server backup/export, execute only reviewed manual SQL, verify starter/reference data, run the reviewed `contacts.child_age` nullable patch before public Contact Us writes to the shared app DB, verify website/app share the intended app DB target, and preserve Quran YouTube/video extract for later Library migration.
