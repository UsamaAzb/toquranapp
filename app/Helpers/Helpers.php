...
CSS;
    ...
    public static function youtubeToEmbed(string $url): string
    {
        ...
        if (preg_match('~youtube(?:-nocookie)?\.com/embed/([^?\s/]+)~', $url, $match)) {
            return 'https://www.youtube.com/embed/'.$match[1];
        }

        // youtube.com/shorts/{id} format
        if (preg_match('~youtube(?:-nocookie)?\.com/shorts/([^?\s/]+)~', $url, $match)) {
            return 'https://www.youtube.com/embed/'.$match[1];
        }
    ...
    public static function trustedVideoEmbedUrl(?string $url): ?string
    {
        $embedUrl = trim(self::youtubeToEmbed((string) $url));
        ...
        if (
            in_array($host, ['youtube.com', 'www.youtube.com', 'youtube-nocookie.com', 'www.youtube-nocookie.com'], true)
            && str_starts_with($path, '/embed/')
        ) {
            return $embedUrl;
        }

        if ($host === 'embed.ted.com' && str_starts_with($path, '/talks/')) {
            return $embedUrl;
        }
    ...
    public static function trustedExternalAttachmentUrl(?string $url): ?string
    {
        $url = trim((string) $url);
