...
class SeriesTaskAssignmentModal extends Component
{
    ...
    /** @var array<int, int> */
    public array $startPositions = [];

    #[On('open-series-task-assignment-modal')]
    ...
    {
        $assignmentService = app(SeriesTaskAssignmentService::class);
        $task = $this->resolveOwnedTaskOrFail((int) $this->taskId)->load(['versions.items']);
        abort_if($task->isArchived(), 404);
        $version = $task->versions->firstWhere('id', $this->activeVersionId);
        abort_if($version === null, 404);

        $allRows = $this->assignmentRows($task, false);
        $this->initializeSelection($allRows, (int) $version->id);
        ...
        $managedStudentIds = $allRows
            ...
            ->values()
            ->all();
        $positions = collect($this->startPositions)
            ->mapWithKeys(fn ($position, $studentId): array => [(int) $studentId => max(1, (int) $position)])
            ->all();
        ...
        $this->dispatch('series-task-assignment-saved');
        $this->close();
    }
    ...
    public function render(): View
    {
        if (! $this->show || $this->taskId === null) {
            return view('livewire.teacher.series-task-assignment-modal', [
                'task' => null,
                'activeVersion' => null,
                'sections' => [],
                'positionOptions' => [],
        ...
        if ($activeVersion) {
            $this->initializeSelection($allRows, (int) $activeVersion->id);
        }
        ...
        return view('livewire.teacher.series-task-assignment-modal', [
            ...
            'sections' => $activeVersion
                ...
                : [],
            'positionOptions' => $activeVersion
                ? $activeVersion->items->where('is_active', true)->sortBy('sequence_position')->values()
                : collect(),
        ]);
    }

    private function assignmentRows(SeriesTask $task, bool $applySearch): SupportCollection
    {
        ...
        $assignments = DB::table('series_task_student_assignments as assignments')
            ->leftJoin('series_task_versions as versions', 'versions.id', '=', 'assignments.version_id')
            ->select(
                'assignments.student_id',
                'assignments.version_id',
                'assignments.start_sequence_position',
                'versions.display_name as version_display_name'
            )
            ->where('assignments.series_task_id', $task->id)
            ->whereIn('assignments.student_id', $studentIds)
            ->whereNull('assignments.effective_to_date')
            ->get()
            ->keyBy('student_id');

        $states = DB::table('series_task_student_generation_states')
            ->where('series_task_id', $task->id)
            ->whereIn('student_id', $studentIds)
            ->get(['student_id', 'next_sequence_position', 'last_generated_date'])
            ->keyBy('student_id');

        $deliveredToday = DB::table('class_sessions')
            ->where('series_task_id', $task->id)
            ->whereIn('student_id', $studentIds)
            ->whereDate('generated_for_date', now(config('app.timezone', 'Africa/Cairo'))->toDateString())
            ->pluck('student_id')
            ->map(fn ($value): int => (int) $value)
            ->flip();

        return $students->map(function (Student $student) use ($assignments, $states, $deliveredToday): array {
            ...
            return [
                ...
                'current_position' => $state?->next_sequence_position
                    ? (int) $state->next_sequence_position
                    : (int) ($assignment?->start_sequence_position ?? 1),
                'delivered_today' => $deliveredToday->has((int) $student->id),
            ];
        })->values();
    }

    private function eligibleStudents(SeriesTask $task, bool $applySearch): SupportCollection
    {
        $query = Student::query()
    ...
    private function buildSections(SupportCollection $rows, int $activeVersionId): array
    {
        return [
            'assigned_here' => [
                'title' => 'Assigned to this pathway',
                'description' => 'Unselect students here to remove them from this pathway for future generation.',
                'rows' => $rows
            ...
            'unassigned' => [
                'title' => 'Not assigned',
                'description' => 'Select students here to assign this pathway.',
                'rows' => $rows
            ...
            'assigned_elsewhere' => [
                'title' => 'Assigned to another pathway',
                'description' => 'Select students here to move them to this pathway for future generation.',
                'rows' => $rows
                    ->filter(fn (array $row): bool => $row['current_version_id'] !== null
    ...
    private function initializeSelection(SupportCollection $rows, int $activeVersionId): void
    {
        foreach ($rows as $row) {
            ...
            if (! array_key_exists($studentId, $this->startPositions)) {
                $this->startPositions[$studentId] = max(1, (int) ($row['current_position'] ?? 1));
            }
        }
    }
    ...
    private function eligibleStudentIdsSubquery(SeriesTask $task): QueryBuilder
    {
    ...
    private function ownedClassSubjectIds(SeriesTask $task): array
    {
