...
class SeriesTaskRecurrenceService
{
    ...
    public function shouldGenerateOn(SeriesTask $task, Carbon $date): bool
    {
    ...
    /** @return string[] */
    public function validateRule(SeriesTask $task): array
    {
        $errors = [];
    ...
    public function candidateDatesForState(
        SeriesTask $task,
        SeriesTaskStudentGenerationState $state,
        Carbon $today
    ): array {
        ...
        for ($offset = 6; $offset >= 0; $offset--) {
            ...
            if ($state->end_date !== null && $candidate->gt(Carbon::parse($state->end_date)->startOfDay())) {
                continue;
            }
    ...
    private function matchesDaily(SeriesTask $task, Carbon $date): bool
    {
    ...
    private function matchesWeekly(SeriesTask $task, Carbon $date): bool
    {
    ...
    private function matchesMonthly(SeriesTask $task, Carbon $date): bool
    {
    ...
    private function resolveStartDate(SeriesTaskStudentGenerationState $state, Carbon $today): Carbon
    {
